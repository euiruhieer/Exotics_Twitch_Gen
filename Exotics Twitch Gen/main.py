...
print("""Made By Exotic 🤝 Discord: https://discord.gg/TX9BbzbU6t """)
...
class Main:
    ...
    def __init__(self):
        self.SetTitle('[Exotic#2021 Twitch Token Gen And Checker]')
        self.clear()
...
Checking
    ...
    def SendWebhook(self,title,message,icon_url,thumbnail_url,proxy,useragent):
        try:
            ...
            message_to_send = {"embeds": [{"title": title,"description": message,"color": 65362,"author": {"name": "GamerYT#3514 DISCORD SERVER [CLICK HERE]","url": "https://discord.gg/fWuVJuTc9D","icon_url": icon_url},"footer": {"text": "MADE BY ","icon_url": icon_url},"thumbnail": {"url": thumbnail_url},"timestamp": timestamp}]}
    ...
    def TitleUpdate(self):
        while True:
            self.SetTitle(f'[Exotic#2021 Twitch Token Checker] ^| HITS: {self.hits} ^| BADS: {self.bads} ^| RETRIES: {self.retries} ^| WEBHOOK RETRIES: {self.webhook_retries} ^| THREADS: {active_count()-1}')
            sleep(0.1)
